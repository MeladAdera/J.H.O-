  ...
  return (
    <div className="z-20 flex flex-col">
      <div className="mb-6 inline-flex items-center gap-2.5 self-start rounded-full border border-black/8 bg-white px-3.5 py-1.5 shadow-sm">
        ...
        <span className="font-atelier-mono text-[10px] uppercase text-atelier-muted">
          {t("badge.bespoke")}
        </span>
      </div>
      ...
      <h1 className="font-atelier-sans text-4xl font-bold uppercase leading-[1.03] tracking-tight text-atelier-dark sm:text-5xl xl:text-6xl 2xl:text-7xl">
        {t("headline.line1")}
        <span className="block py-1 pe-2 font-atelier-serif text-5xl font-normal italic lowercase tracking-normal text-atelier-pink sm:text-6xl xl:text-7xl 2xl:text-8xl">
          {t("headline.line2")}
        </span>
      </h1>
      ...
      <div className="mt-10 grid grid-cols-3 gap-4 border-t border-black/8 pt-6 font-atelier-mono text-[11px]">
        {PHASES.map(({ key, accent }, index) => (
          <div key={key}>
            ...
            <div className="mt-0.5 text-[10px] text-atelier-muted">{t(`phases.${key}.note`)}</div>
          </div>
        ))}
      </div>
    </div>

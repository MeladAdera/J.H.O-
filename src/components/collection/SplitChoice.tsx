  ...
  return (
    <section className="relative flex h-[calc(100vh-60px)] w-full flex-col overflow-hidden md:flex-row">
      ...
      <div className="pointer-events-none absolute top-5 left-1/2 z-30 -translate-x-1/2 text-center">
        <div className="flex items-center gap-2 rounded-full border border-white/10 bg-black/85 px-4 py-1.5 font-archive-mono text-[9px] uppercase tracking-[0.3em] text-archive-muted shadow-2xl backdrop-blur-md">
      ...
      {panels.map((panel, index) => {
        ...
        return (
          <button
            ...
            className={[
              "split-panel group relative h-1/2 w-full flex-1 cursor-pointer overflow-hidden bg-archive-deep text-start md:h-full md:w-1/2",
              index === 0 ? "border-b border-white/10 md:border-b-0 md:border-e" : "",
          ...
          >
            ...
            <div className="absolute inset-0 overflow-hidden">
              ...
              />
              <div className="absolute inset-0 bg-linear-to-t from-archive-black via-archive-black/45 to-archive-black/20 opacity-85 transition duration-700 group-hover:opacity-60" />
              <div
            ...
            <div
              className={`absolute inset-s-7 top-7 z-20 flex items-center gap-2.5 font-archive-mono text-[9px] uppercase tracking-[0.25em] text-archive-muted ${
                index === 1 ? "md:top-16" : ""
              }`}
            >
            ...
            {panel.coordinates && (
              <div className="pointer-events-none absolute bottom-32 inset-s-8 z-20 hidden lg:block">
                <p className="text-vertical latin-telemetry font-archive-mono text-[9px] uppercase tracking-[0.45em] text-white/35">
                  {panel.coordinates}
                </p>
              </div>
            )}
            ...
            <div className="absolute inset-x-6 bottom-8 z-20 flex items-end justify-between transition duration-500 md:inset-x-14">
              <div className="max-w-md transform transition duration-500 group-hover:-translate-y-1">
                <span className="mb-1.5 block font-archive-mono text-[10px] uppercase tracking-[0.3em] text-archive-pink">
                  {t(`${panel.key}.kicker`)}
                </span>
                <h2 className="mb-3 font-archive-serif text-5xl font-light uppercase tracking-wide text-archive-chalk transition duration-300 group-hover:text-white sm:text-7xl lg:text-8xl">
                  {t(`${panel.key}.title`)}
                </h2>
                <p className="line-clamp-2 text-xs font-light leading-relaxed tracking-wide text-archive-muted sm:text-sm md:line-clamp-none">
                  {t(`${panel.key}.description`)}
                </p>
              </div>

              <div className="circle-reveal-btn ms-4 flex h-12 w-12 shrink-0 items-center justify-center rounded-full border border-white/25 bg-black/40 text-archive-chalk backdrop-blur-md group-hover:shadow-[0_0_20px_rgba(255,117,151,0.4)]">
                <svg
